...
try {
    ...
    setTimeout(() => {
        ...
        if (
            ...
            status.textContent =
                "ERROR: WebSocket server is unavailable.";
        }
    }, 10000);
...
loadSecret(existingSecret).catch(() => {
    console.warn("Invalid room secret");
});
```
...
try {
    ...
    packet.set(iv, 0);
    packet.set(encryptedBytes, 12);

    const encoded = bytesToBase64(packet);

    const total =
        Math.ceil(encoded.length / 48000);
...
console.error("Unexpected binary message");
return;
}
...
if (msg.type === "chunk") {
    ...
    if (
        ...
        incoming.parts[msg.index] = msg.data;
    ...
    if (
        ...
        console.error(
            "Missing chunks",
            {
                ...
                received: incoming.receivedParts
            }
        );
